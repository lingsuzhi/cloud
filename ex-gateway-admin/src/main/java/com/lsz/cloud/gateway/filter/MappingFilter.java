package com.lsz.cloud.gateway.filter;

import com.lsz.cloud.gateway.wrapper.RequestWrapper;
import com.lsz.cloud.gateway.wrapper.WrappedHttpResponse;
import com.lsz.cloud.util.UuidMd5;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain
        ...
        for (String item : serviceList) {
            if (uri.startsWith(item)) {
                ...
                zuulRequestHeaders.put("SZ-caller", "ex-gateway-admin");
                zuulRequestHeaders.put("SZ-threadName", threadName);
                fakeRequest.setRequestURI("/ex-" + uri);
                break;
            }
        }
        if (fakeRequest != null) {
